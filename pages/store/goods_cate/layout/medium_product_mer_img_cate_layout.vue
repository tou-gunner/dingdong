<!-- 包含中图商品的分类页面布局，分类列表含图片 -->
<template>
  <view class="medium-product-layout">

    <!-- 顶部搜索组件 -->
    <TopSearch theme="light" />

    <!-- 横向图片分类列表组件 -->
    <HorizontalImgCateList :list="categoryList" :activeIndex="levelFirstCateIndex"
      @toggle="handleToggleFirstCategory" v-if="categoryList.length" />
    <view class="main-layout flex flex-1">
      ...
      <view class="main-right-layout flex-full-h">
        ...
        <view class="flex-full-h">
          ...
          <!-- 中图商品列表组件 -->
          <MediumProductList :list="productList" @loadMore="handleLoadMoreProduct" @addCart="selecAttr"
...
import MediumProductList from '@/components/goodsCate/medium-product-list.vue';
...
export default {
  name: "MediumProductImgCateLayout",
