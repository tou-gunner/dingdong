...
export default {
	...
	props: {
		...
		explain: {
			type: String,
			default: 'ເລື່ອນໄປທາງຂວາເພື່ອສຳເລັດການຢືນຢັນ'
		},
		imgSize: {
			type: Object,
			default() {
	...
	data() {
		return {
			...
			defaultImg: '',
			captchaType: app.globalData.captcha_type || 'clickWord',
		};
	},
	watch: {
		captchaType: {
	...
	methods: {
		...
		show() {
			// console.log(app.globalData.captcha_type);
			this.captchaType = app.globalData.captcha_type;
			switch (this.captchaType.toString()) {
				case 'blockPuzzle':
					this.verifyType = '2';
					this.componentType = 'VerifySlide';
					break;
