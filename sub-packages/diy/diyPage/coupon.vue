<template>
	<!-- 优惠券 -->
	<view :style="[couponWrapStyle]" class="relative coupon-wrap">
		<view class="coupon-wrap" :style="[couponWrapBgColor]" v-if="couponList.length">
			<view v-if="dataConfig.styleConfig.tabVal == 0" class="coupon1">
				<scroll-view scroll-x="true">
					<view class="list acea-row p-24">
						<view v-for="item in couponList" :key="item.coupon_id" class="item" :style="[couponItemFirstStyle]">
							<view class="text">
			...
			<view v-else-if="dataConfig.styleConfig.tabVal == 1" class="coupon2">
				<scroll-view scroll-x="true">
					<view class="list acea-row p-24">
						<view v-for="item in couponList" :key="item.coupon_id" class="item">
			...
			<view v-else-if="dataConfig.styleConfig.tabVal == 2" class="coupon3 rd-16rpx" :style="[coupon3Color]">
				<scroll-view scroll-x="true">
					<view class="list acea-row  p-24">
						<view v-for="item in couponList" :key="item.coupon_id" class="item">
			...
			<view v-else-if="dataConfig.styleConfig.tabVal == 4" class="coupon5 rd-16rpx">
				<scroll-view scroll-x="true">
					<view class="list acea-row pt-24 pb-24 pr-20 pl-20">
						<view v-for="item in couponList" :key="item.coupon_id" class="item acea-row" :style="[bntBgColor]">
							<view class="text acea-row row-column row-middle row-center">
								<view class="money" :style="[couponMoneyColor]">
									<template>
										₭<text class="number">{{ item.coupon_price }}</text>
...
export default {
	...
	computed: {
		...mapGetters(['isLogin']),
		coupon4Style() {
			const width = uni.upx2px(296);
			const height = uni.upx2px(180);
			const arcX = uni.upx2px(20);
			return {
				clipPath: `path("M0,0  Q${arcX}, ${height / 2} 0, ${height} L${width}, ${height} L${width}, 0 Z")`
			};
		},
		couponMoneyColor() {
			let styleObject = {};
		...
		couponWrapStyle() {
			return {
				...
				'--item-box-shadow': diyUtil.buildShadowStyle(this.dataConfig.couponShadowConfig),
				'--item-gap': `${this.dataConfig.spacingConfig.val}px`,
			};
		},
	},
	created() {
	...
	methods: {
		getUseCouponUrl(item) {
			return `/pages/columnGoods/goods_coupon_list/index?coupon_id=${item.coupon_id}`;
		},
		getCoupon: function () {
			let that = this;
			getCouponData({
				...
				page: 1,
				limit: that.dataConfig.numberConfig.val || 10
			}).then(res => {
		...
		receiveCoupon(item) {
			...
			if (!that.isLogin) {
				...
				setCouponReceive(item.coupon_id).then(res => {
					...
					setTimeout(() => {
						const url = that.getUseCouponUrl(item);
						uni.navigateTo({
							url
						})
					}, 500);
		...
		goCoupon() {
			const targetUrl = this.parentMerId ? `/pages/store/home/coupon?id=${this.parentMerId}` : '/pages/activity/collect_coupons/index';
			uni.navigateTo({
				url: targetUrl
			})
		},
...
.last-item-placeholder {
	width: calc(24rpx - var(--item-gap, 12rpx));
	flex-shrink: 0;
}
...
.coupon1 {
	...
	.item {
		...
		border-radius: 32rpx;
		margin-top: 20rpx;
		margin-right: var(--item-gap, 12rpx);
		background: var(--view-theme);
...
.coupon3 {
	...
	.inner {
		...
		border: 1px solid var(--coupon3-color);
	}
...
.coupon5 {
	...
	.item {
		...
		border-radius: 12rpx;
		margin-right: var(--item-gap, 12rpx);
		background: linear-gradient(0deg, var(--view-theme) 0%, var(--view-assist) 100%);
	}
