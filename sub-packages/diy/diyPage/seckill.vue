<template>
	<!-- 秒杀列表 -->
	<view :style="[boxStyle]" v-if="spikeList.length > 0" class="seckill-wrapper relative">
		<view :style="[wrapperStyle]">
			...
			<view class="w-full h-96 px-24 flex-between-center bg-cover" :style="[headerStyle]">
				...
				<view class="flex-y-center fs-24 text--w111-999" :style="[headerBntColor]"
					...
					<text>{{ dataConfig.rightBntConfig.value }}</text>
			...
			<view class="pt-32 pr-20 pb-32 pl-20 bg--w111-fff" :style="[boxContentStyle]" v-if="goodStyleConfig == 0">
				<view class="w-full flex justify-between item" v-for="(item, index) in spikeList" :key="index"
					@tap="goDetails(item)" :style="[goodStyle]">
					<image :src="item.image" :style="{ width: '240rpx', height: '240rpx', borderRadius: imgStyle }"></image>
			...
			<view class="grid-column-2 grid-gap-22rpx pt-32 pr-20 pb-32 pl-20" :style="[boxContentStyle]"
				...
				<view v-for="(item, index) in spikeList" :key="index" @tap="goDetails(item)" :style="[goodStyle]">
					<image :src="item.image" :style="{ width: '100%', height: '324rpx', borderRadius: imgStyle }"></image>
					...
					<view class="flex justify-between items-end mt-10">
						...
						<view class='w-144 h-56 rd-30rpx flex-center fs-24 text--w111-fff bg--w111-E93323' v-if="!showBtn"
							:style="[btnBgColor, getBtnOtherStyle(item)]">{{ item.skill_status | btnText }}</view>
					</view>
				</view>
			</view>
			<!-- 三列 -->
			<view class="grid-column-3 grid-gap-18rpx pt-32 pr-20 pb-32 pl-20" :style="[boxContentStyle]"
				...
				<view v-for="(item, index) in spikeList" :key="index" @tap="goDetails(item)" :style="[goodStyle]">
					<image :src="item.image" :style="{ width: '100%', height: '212rpx', borderRadius: imgStyle }"></image>
					...
					<view class="price-box flex-center relative" :style="[tagBag]" v-if="checkboxInfo.includes(2)">
						<view :style="{ color: bargainPriceColor }">
							<priceFormat :price="item.price" intSize="30" decimalSize="30" labelSize="24"></priceFormat>
						</view>
			...
			<scroll-view scroll-x="true" :show-scrollbar="false" class="white-nowrap vertical-middle w-full p-32 border-box"
				...
				<view style="white-space: nowrap;">
					<view class="inline-block" :class="{ 'ml-20': index }" v-for="(item, index) in spikeList" :key="index"
						@tap="goDetails(item)" :style="[goodStyle]">
						<image :src="item.image" :style="{ width: '224rpx', height: '224rpx', borderRadius: imgStyle }"></image>
						<view class="w-222 line1 mt-16 fs-26 lh-26rpx" :style="[productStyle]" v-if="checkboxInfo.includes(0)">
							{{ item.store_name }}</view>
...
export default {
	...
	computed: {
		...
		boxContentStyle() {
			return {
				background: diyUtil.buildLinearColor(this.dataConfig.moduleColor),
			};
		},
		/*商品模板*/
		goodStyleConfig() {
		...
		btnBgColor() {
			return {
				background: this.dataConfig.toneConfig.tabVal ? `linear-gradient(90deg,${this.dataConfig.goodsBntColor.color[1].item} 0%,${this.dataConfig.goodsBntColor.color[0].item} 100%)` : 'linear-gradient(90deg, var(--view-assist) 0%, var(--view-theme) 100%)',
				color: this.dataConfig.goodsBntTxtColor.color[0].item
			}
		},
		progressBgColor() {
			return this.dataConfig.toneConfig.tabVal ? `linear-gradient(90deg,${this.dataConfig.progressColor.color[0].item} 0%,${this.dataConfig.progressColor.color[1].item} 100%)` : 'linear-gradient(45deg, var(--view-assist) 0%, var(--view-theme) 100%)'
		},
		/*倒计时背景色*/
		numberBgColor() {
			const color = this.styleConfig ? this.dataConfig.numberBgColor : this.dataConfig.numberBgColor2;
			return {
				"--dot-color": this.dotColor,
				"--time-bg": diyUtil.buildLinearColor(color)
			};
		},
		numberColor() {
		...
		goodStyle() {
			return {
				boxShadow: diyUtil.buildShadowStyle(this.dataConfig.goodsShadowConfig)
			};
		}
	},
	created() {
	...
	methods: {
		getBtnOtherStyle(good) {
			const style = {};
			if (good.skill_status === 0) style.opacity = 0.5;
			return style;
		},
		goPage(url) {
			uni.navigateTo({
		...
		async getSeckillIndexTime() {
			try {
				const res = await getSeckillIndexTime();
				const { seckillTime, seckillTimeIndex } = res.data;
		...
		getSeckillData() {
			...
			getSeckillData({
				...
				unique: that.dataConfig.timestamp,
				mer_id: that.parentMerId || "",
				limit: that.dataConfig.numberConfig.val
			}).then(res => {
				that.datatime = res.data && res.data.stop;
				that.spikeList = res.data.list.map(item => {
...
.price-box {
	width: 150rpx;
	height: 44rpx;
	background: linear-gradient(45deg, var(--view-assist) 0%, var(--view-theme) 100%);
