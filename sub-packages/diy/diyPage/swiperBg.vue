<template>
	<!-- 带背景轮播图 -->
	<view class="swiperBg relative" :style="[swiperBgStyle]">
		<template v-if="dataConfig.swiperConfig.list.length">
			<view class="swiper" :style="[colorBgStyle]" :class="{ 'has-padding': dataConfig.styleConfig.tabVal === 1 }">
				<swiper :style="'height:' + imageH + 'rpx;'" autoplay :previous-margin="swiperMargin"
					:next-margin="swiperMargin" :circular="circular" :interval="interval" :duration="duration" @change="bannerfun"
					v-if="imageH">
					<swiper-item v-for="(item, index) in dataConfig.swiperConfig.list" :key="index">
						<view @click="goDetail(item)" class="swiper-item"
							:style="[itemStyle, active == index ? activeStyle : '', getSwiperStyle(index)]">
							<image :src="$util.BuildImgUrl(item.img)" mode="aspectFill" class="image" :style="[imageStyle]"></image>
						</view>
					</swiper-item>
				</swiper>
				...
				<view class="dot acea-row" :style="[dotStyle]">
					...
					<view class="acea-row"
						:class="{ small: dataConfig.docConfig.tabVal == 1, line: dataConfig.docConfig.tabVal == 3 }" v-else>
						<view class="dot-item" v-for="(item, index) in dataConfig.swiperConfig.list" :key="index"
							:class="{ active: active == index }" :style="[active == index ? dotItemActiveStyle : dotItemStyle]">
						</view>
					</view>
				</view>
			</view>
...
export default {
	...
	inject: ["parentMerId", "parentDiyId"],
	data() {
		return {
			circular: true,
			autoplay: true,
	...
	computed: {
		itemStyle() {
			...
			const top = this.dataConfig.topConfig.val;
			const bottom = this.dataConfig.bottomConfig.val
			const style = {
				paddingTop: `${top * 2}rpx`,
				paddingBottom: `${bottom * 2}rpx`,
				transform: `scale(${num})`
			}
			const type = this.dataConfig.styleConfig.tabVal

			if (type == 1) {
				style.padding = "32rpx 20rpx";
			}
		...
		swiperBgStyle() {
			const paddingInline = this.dataConfig.prConfig.val * 2
			const style = {
				paddingInline: `${paddingInline}rpx`,
			...
			if (this.dataConfig.styleConfig.tabVal !== 1) {
				style.background = this.dataConfig.bgColor.color[0].item;
			}
		...
		colorBgStyle() {
			let styleObject = {
				"--bg-color": this.dataConfig.bgColor.color[0].item
			};
			return styleObject;
		},
		imageStyle() {
	...
	mounted() {
		if (this.imgUrls.length) {
			...
			this.$nextTick((e) => {
				uni.getImageInfo({
					src: that.setDomain(that.$util.BuildImgUrl(that.imgUrls[0].img)),
					success: (res) => {
						if (res && res.height > 0) {
	...
	methods: {
		getSwiperStyle(index) {
			if (this.dataConfig.styleConfig.tabVal == 2) {
				const val = this.dataConfig.imgConfig.val;
				const style = {
					paddingInline: `${val / 2}px`
				};
				if (index !== this.active) {
					const length = this.dataConfig.swiperConfig.list.length;
					const isInCurrentLeft = (index + 1 === length && (index < this.active || this.active === 0)) || index + 1 === this.active;
					style.transformOrigin = `${isInCurrentLeft ? "right" : "left"} center`
				}
				return style;
			}
		...
		goDetail(url) {
			const urls = diyUtil.checkUrl(url.info[0].value, this.parentMerId);
